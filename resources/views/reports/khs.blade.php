...
<body>
    ...
    <table class="course-table">
        ...
        <tbody>
                ...
                @if ($detail->nilai)
                    <tr>
                        ...
                        <td>{{ $detail->mataKuliah->nama_mk }}</td>
                                            ...
                                            <td>{{ $detail->nilai->nilai_huruf }}</td>
                                            <td>{{ $detail->nilai->nilai_angka }}</td>
                                        </tr>
                                        @php
                                            $totalSks += $detail->mataKuliah->sks;
                                            $totalBobot += ($detail->nilai->nilai_angka * $detail->mataKuliah->sks);
                                        @endphp                @endif
            @endforeach
            <tr>
            ...
            <tr>
                <td colspan="3" style="text-align: right;">Indeks Prestasi Semester (IPS)</td>
                <td colspan="3">{{ $totalSks > 0 ? number_format($totalBobot / $totalSks, 2) : '0.00' }}</td>
            </tr>
        </tbody>
    </table>
